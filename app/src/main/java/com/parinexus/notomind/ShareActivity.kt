...
import com.parinexus.designsystem.theme.SkTheme
...
//    val shareViewModel= hiltViewModel<DetailViewModel>()
                    ...
                    EditLabels(
                        ...
                        checks = (uiState as SharedActivityUiState.Success).notoMind.labels,
...
//        },
        ...
        LazyColumn(
            ...
            if (success.notoMind.images.isNotEmpty()) {
                item {
                    Box {
                        success.notoMind.images.reversed().chunked(3).forEach { imageList ->
                            Row(
                                modifier = Modifier
                                    .fillMaxWidth()
                                    .height(200.dp),
            ...
            if (success.notoMind.labels.isNotEmpty()) {
                item {
                    FlowRow(
                        modifier = Modifier.padding(4.dp),
                        horizontalArrangement = Arrangement.spacedBy(8.dp),
                    ) {
                        success.notoMind.labels.forEach {
                            LabelCard(
                                name = it.label,
